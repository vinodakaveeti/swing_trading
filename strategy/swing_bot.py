...
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8851624105:AAGDJ0zxNKS227yEeuMYpqoBfVnyt9a9oXA")  # Get from @BotFather
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "-5402763384")  # Your chat ID
...
def open_position(symbol: str, side: str, entry_price: float, api: YahooFinanceAPI):
    ...
    if TELEGRAM_ENABLED:
        ...
        trade_msg += f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
def check_position_exits(symbol: str, current_price: float):
    ...
    if exit_price is not None:
        ...
        if TELEGRAM_ENABLED:
            ...
            close_msg += f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
def main():
    ...
    if TELEGRAM_ENABLED:
        ...
        startup_message += f"• Timeframe: {CHART_INTERVAL}\n"
    ...
    try:
        while not _shutdown_requested:
            ...
            if TELEGRAM_ENABLED:
                ...
                telegram_message += f"• Time: {start_time}\n"
                telegram_message += f"• Monitoring: {len(WATCH_LIST_INFO)} symbols\n"
                telegram_message += f"• Timeframe: {CHART_INTERVAL}\n"
            ...
            if TELEGRAM_ENABLED:
                ...
                telegram_message += f"• Started: {start_time_str}\n"
                telegram_message += f"• Ended: {end_time_str}\n"
